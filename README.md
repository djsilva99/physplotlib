...
The physplot package allows one to load data from text files, to make simple treatments and to plot data in a fast and efficient way. It is usefull when handling with output files from numerical simulations in a quick way or if one wants to make a quick plot from text files to watch the content. 
...
All files to be used by the package must be located inside the physplot directory.
